...
const Archive=() => {
  ...
  const fetchNotes = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/notes/note/archived`, {
        headers: {
  ...
  const HandleUnArchive = async (noteId) => {
    try{
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/notes/${noteId}/unarchive`,
        {},
        {
          headers: { 
  ...
  // console.log(noteId);
  try{
    const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/notes/${noteId}/trash`,
      {},
      {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('Token')}`
